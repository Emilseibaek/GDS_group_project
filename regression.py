...
def logregExtended(df):
    ...
    y_val_pred = model.predict(X_val)
    print("logreg extended Validation F1:",f1_score(Y_val, y_val_pred,average='binary',pos_label="fake"))
    y_test_pred = model.predict(X_test)
    print("logreg extended Test F1:", f1_score(Y_test,y_test_pred,average="binary",pos_label="fake"))
